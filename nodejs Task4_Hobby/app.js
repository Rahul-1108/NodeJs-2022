...
//AsyncLoop Hobby get 
//============================================
const get_hobby_data = function(params,callback){

con.query("select GROUP_CONCAT(hobby) as hobby from tbl_hobby where id='"+params['id']+"'", function (err, result) {
  if(result[0] != undefined){

      callback(result[0].hobby)
  }else{
      callback('')
  }
...
app.get('/',upload.single('file'), function (req, res) {
  ...
  con.query("SELECT * FROM tbl_user_data", function (err, result) {
    ...
    asyncLoop(result, function (item, next) {
      ...
      get_hobby_data(item,function(data){
          item.hobby = data;
...
/* app.get('/get_data', function (req, res) {
  ...
  con.query("select * from tbl_user ", function (err, result, fields) {
      ...
      asyncLoop(result, function (item, next) {
          ...
          console.log(result)
          res.render(path.join(__dirname + '/show_data.ejs'), { data: result })
      });
  });
})
 */